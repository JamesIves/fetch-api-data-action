import { info, exportVariable, setOutput as setEnvironmentOutput, debug } from '@actions/core';
 ...
 */
export async function retrieveData({ debug: requestDebug, endpoint, configuration, auth, isTokenRequest, retry }) {
    try {
        info(isTokenRequest
            ? 'Fetching credentials from the token endpoint… 🎟️'
            : 'Fetching the requested data… 📦');
        const settings = configuration
            ? JSON.parse(mustache.render(configuration, auth ? parseData(auth) : null))
            : {};
        if (settings.body) {
 ...
 */
export async function generateExport({ data, encoding, format, saveLocation, saveName, setOutput, variableName }) {
    ...
    try {
        ...
        throw new Error(`There was an error generating the export file: ${error} ❌`, { cause: error });
    }
}
